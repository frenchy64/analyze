...
(def ^:dynamic *analyzer-warn-on-undeclared* true)
...
(defn confirm-var-exists [env prefix suffix]
  (when *analyzer-warn-on-undeclared*
    (let [crnt-ns (-> env :ns :name)
          _ (assert crnt-ns "Missing namespace name")]
      (when (= prefix crnt-ns)
...
(defn resolve-alias 
  "Resolve class or namespace alias"
  [env name]
  (let [nsym (symbol name)]
    (cond
      (-> env :ns :imports (get nsym))
      {:class (-> env :ns :imports (get nsym))}

      (-> env :ns :requires (get nsym))
      {:ns (-> env :ns :requires (get nsym))}

      (find-ns nsym)
      {:ns nsym}

      (resolve-class env (symbol (str "java.lang." name)))
      {:class (symbol (str "java.lang." name))}

      (resolve-class env nsym)
      {:class nsym}

      :else (do
              (binding [*out* *err*]
                (println (str "WARNING: Use of undeclared namespace " nsym)))
              {:ns nsym}))))
...
(defn resolve-existing-var [env sym]
  (let [s (str sym)
        ...
        (cond
          ...
          ;; static method/field or var
          (namespace sym)
          (let [ns (namespace sym)
                resolved (resolve-alias env ns)]
            (when (:ns resolved)
              (confirm-var-exists env (:ns resolved) (symbol (name sym))))
            (symbol (str (or (:ns resolved) (:class resolved)) "/" (name sym))))
          ...
          (-> env :ns :uses sym)
          (symbol (str (-> env :ns :uses sym) "/" (name sym)))

          (-> env :ns :imports sym)
          (-> env :ns :imports sym)

          (resolve-class env (symbol (str "java.lang." sym)))
          (symbol (str "java.lang." sym))

          (and (not (contains? (-> env :ns :excludes) sym))
               (get (ns-publics 'clojure.core) sym))
          (symbol (str "clojure.core" "/" sym))
  ...
  (let [s (str sym)
        ...
        (cond
         ...
         (let [ns (namespace sym)
           ...
           (symbol (str (resolve-alias env ns) "/" (munge (name sym)))))
...
;; Excludes
(analyze '{:ns {:excludes #{+}}} '(+ 1 1))
(analyze '{:ns {:name blah.blah :excludes #{+}}} '(+ 1 1))
(analyze '{:ns {:name blah :excludes #{+}}} '(= 1 1))
  )
